    ...
    stage('Get CodeDx Project ID') {
      steps {
        sh '''
          ...
          CODEDX_PROJECT_ID=$(./getProjectID.sh --url=${CODEDX_SERVER_URL} --apikey=${CODEDX_ACCESS_TOKEN} --project=${IO_POC_PROJECT_NAME})
          echo "CodeDx Project ID = $CODEDX_PROJECT_ID"
        '''
        script {
          env.CODEDX_PROJECT_ID = sh(script:'./getProjectID.sh --url=${CODEDX_SERVER_URL} --apikey=${CODEDX_ACCESS_TOKEN} --project=${IO_POC_PROJECT_NAME}', returnStdout: true).trim()
        }
      }
    }
